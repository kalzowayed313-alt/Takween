  ...
  return (
    <div onClick={onClick} className="bg-white dark:bg-slate-900 rounded-3xl border shadow-sm hover:shadow-2xl transition-all p-6 group cursor-pointer active:scale-95 border-b-4 border-transparent hover:border-blue-600">
      <div className="flex items-start justify-between mb-4">
        <img src={employee.avatar} alt="" className="w-16 h-16 rounded-2xl object-cover ring-4 ring-slate-50 shadow-sm" />
      ...
      {activeTab === 'ACTIVE' ? (
        ...
        <div className="space-y-4">
          {pendingEmployees.length > 0 ? pendingEmployees.map(emp => (
            <div key={emp.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 animate-in slide-in-from-right duration-300">
              <div className="flex items-center gap-4">
                <img src={emp.avatar} className="w-14 h-14 rounded-2xl object-cover" alt="" />
                <div>
